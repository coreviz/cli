...
/**
 * In-process storage for device auth sessions between login → login_wait calls.
 * Keyed by 'current' (one active session at a time is sufficient).
 */
const pendingDeviceAuths = new Map();
...
/**
 * Like safe(), but also checks that ctx.sdk is initialized before running.
 * Use for any tool that makes cloud API calls.
 */
function safeCloud(ctxRef, fn) {
    return safe(async (args) => {
        if (!ctxRef.sdk) {
            return {
                content: [{
                    type: 'text', text: JSON.stringify({
                        error: 'not_authenticated',
                        message: 'Not logged in to CoreViz. Call the login tool to authenticate.',
                    })
                }],
            };
        }
...
/**
 * Create a collection, directing it to the target org stored in sync state when set.
 * Uses sdk.collections.create(name, icon, organizationId) — the SDK resolves the org
 * from /api/me when organizationId is omitted.
 *
 * Resilient to SDK/server response shape mismatches: if the create call returns
 * something without an `.id` (e.g. server returns the collection directly rather
 * than wrapped in `{ dataset: ... }`), we fall back to listing collections and
 * finding the newly-created one by name.
 */
async function createCollection(sdkInstance, state, name) {
    const orgId = getTargetOrgId(state) || undefined;
    let result;
    try {
        result = await sdkInstance.collections.create(name, 'folder', orgId);
    } catch (err) {
        // Creation may have succeeded server-side but failed to parse the response.
        // Fall through to the list-based fallback below.
        result = null;
    }
    if (result?.id) return result;

    // Fallback: list collections and find the newly-created one by name.
    const collections = await sdkInstance.collections.list(orgId);
    const found = collections.find(c => c.name === name);
    if (found) return found;

    throw new Error(`Collection "${name}" could not be created or found. API response: ${JSON.stringify(result)}`);
}
...
async function uploadAndEnrich(sdk, dir, filename, state) {
    ...
    let uploadPath;  // undefined = collection root
    if (subdir && subdir !== '.') {
    ...
    // Normalize response: server may return { mediaId } or { id } depending on version
    const mediaId = result?.mediaId || result?.id;
    if (!mediaId) throw new Error(`Upload succeeded but server returned no media ID. Response: ${JSON.stringify(result)}`);

    markSynced(state, filename, mediaId, hash);

    // Pull enrichment (best-effort — indexing may still be in progress)
    const enriched = await fetchEnrichment(sdk, mediaId);
 ...
 * @param {{ sdk: import('@coreviz/sdk').CoreViz | null }} ctx  Mutable SDK context (replaced after login)
 * @param {string} dir  Absolute path to the watched local directory
 */
export function registerLocalTools(server, ctx, dir) {

    /** Return a standard "not authenticated" response */
    function notAuthedResponse() {
        return text({
            error: 'not_authenticated',
            message: 'Not logged in to CoreViz. Call the login tool to authenticate.',
        });
    ...
    /** Shorthand: ctx.sdk with auth guard */
    function sdk() {
        return ctx.sdk;
    }
    ...
    server.tool(
        'login',
        'Start CoreViz authentication via device authorization. Returns a URL and short code — show them to the user so they can approve in their browser. After showing the user the URL and code, call login_wait to complete authentication once they have approved.',
        {},
        safe(async () => {
            const { config, baseUrl, buildSdk } = await import('../bin/local-mcp.js');
            ...
            if (error || !data) {
                return text({ error: 'Failed to start login flow', detail: error?.message || 'No data received' });
            }

            const { verification_uri, user_code, device_code, interval = 5, expires_in } = data;

            // Store session for login_wait to pick up
            pendingDeviceAuths.set('current', {
                authClient,
                device_code,
                interval,
                buildSdk,
            ...
            return text({
                status: 'pending',
                action: 'Show the user this URL and code. Once they approve in their browser, call login_wait to complete authentication.',
    ...
    server.tool(
        'login_wait',
        'Complete CoreViz authentication after the user has approved the device code in their browser. Call this after login returns a URL and code. Returns status "pending" if the user has not approved yet — call again after a few seconds. Returns status "authenticated" when done.',
        {},
        safe(async () => {
            const pending = pendingDeviceAuths.get('current');
            if (!pending) {
                return text({ error: 'No pending login session. Call login first to start a new one.' });
            }

            if (Date.now() > pending.expiresAt) {
                pendingDeviceAuths.delete('current');
                return text({ error: 'Login code expired. Call login to start a new session.' });
            }

            const { authClient, device_code, buildSdk, config } = pending;

            const { data: token, error: pollError } = await authClient.device.token({
                client_id: 'coreviz-cli',
                device_code,
            ...
            if (token) {
                pendingDeviceAuths.delete('current');
                config.set('session', token);
                ctx.sdk = buildSdk(token.access_token, null);
                const me = await ctx.sdk.me();
                return text({
                    status: 'authenticated',
                    user: me.name,
                    email: me.email,
                    organization: me.organizationName || me.organizationId,
                });
            }
            ...
            if (pollError) {
                switch (pollError.error) {
                    case 'authorization_pending':
                        return text({ status: 'pending', message: 'User has not approved yet. Call login_wait again in a few seconds.' });
                    case 'slow_down':
                        pending.interval += 5;
                        return text({ status: 'pending', message: 'Polling too fast. Wait a bit longer and call login_wait again.' });
                    case 'access_denied':
                        pendingDeviceAuths.delete('current');
                        return text({ status: 'denied', error: 'Access denied by user.' });
                    case 'expired_token':
                        pendingDeviceAuths.delete('current');
                        return text({ status: 'expired', error: 'Code expired. Call login to start a new session.' });
                    default:
                        return text({ status: 'error', error: pollError.error_description || pollError.message || pollError.error });
                }
            }

            return text({ status: 'pending', message: 'No response yet. Call login_wait again.' });
        })
    );

    server.tool(
        'whoami',
        'Show the currently authenticated CoreViz user, organization, and which collection this folder is connected to.',
        {},
        safe(async () => {
            if (!sdk()) return notAuthedResponse();
            const me = await sdk().me();
            const state = loadSyncState(dir);
            const targetOrgId = getTargetOrgId(state);
            return text({
                user: me.name,
                email: me.email,
                defaultOrganization: me.organizationName || me.organizationId,
                defaultOrganizationId: me.organizationId,
                targetOrganization: state.organizationName ?? null,
                targetOrganizationId: targetOrgId ?? me.organizationId,
                note: targetOrgId && targetOrgId !== me.organizationId
                    ? `Uploads are going to "${state.organizationName || targetOrgId}" (overridden). Call switch_org to change.`
                    : 'Uploads go to your default organization. Call switch_org to change.',
                folder: dir,
                connectedCollection: state.collectionName ?? null,
    ...
    server.tool(
        'switch_org',
        'Change which CoreViz organization this folder syncs to. Lists your available organizations so you can pick one. If you already know the org ID, pass it directly to skip the listing.',
        {
            organizationId: z.string().optional().describe('The organization ID to switch to. If omitted, lists available organizations instead.'),
        },
        safe(async ({ organizationId }) => {
            if (!sdk()) return notAuthedResponse();

            if (!organizationId) {
                const [orgs, me] = await Promise.all([sdk().organizations.list(), sdk().me()]);
                return text({
                    availableOrganizations: orgs.map(o => ({
                        id: o.id,
                        name: o.name,
                        slug: o.slug,
                        current: o.id === me.organizationId,
                    })),
                    usage: 'Call switch_org({ organizationId: "..." }) with one of the IDs above.',
                });
            }

            // Switch to the given org
            const state = loadSyncState(dir);

            // Verify the org exists by fetching its datasets
            let orgName = organizationId;
            try {
                const collections = await sdk().collections.list(organizationId);
                // If that worked, it's a valid org — try to get a name from a collection's org field
                orgName = collections[0]?.organizationName || organizationId;
            } catch (err) {
                return text({ error: `Could not access organization "${organizationId}": ${err.message}` });
            }

            setTargetOrgId(state, organizationId, orgName);

            // If there's an existing collection tied to a different org, clear it so
            // the next sync creates a fresh collection in the new org.
            if (state.collectionId) {
                const me = await sdk().me();
                if (state.organizationId !== me.organizationId) {
            ...
            return text({
                ...
                note: state.collectionId
                    ? `Future uploads will go to this organization. Existing collection "${state.collectionName}" is still linked.`
                    : 'Future uploads will create a new collection in this organization.',
            });
        })
    );
    ...
    server.tool(
        'upload_file',
        'Upload a single local file to CoreViz cloud for processing. The file will be sent to CoreViz servers where it will be indexed (embeddings, auto-description, object detection, etc). Only call this after the user has agreed to send the file to the cloud.',
        {
            filename: z.string().describe('Local filename or relative path to upload (e.g. "photo.jpg" or "vacation/beach.jpg")'),
        },
        safeCloud(ctx, async ({ filename }) => {
            let state = loadSyncState(dir);

            // Create collection on first upload
            if (!state.collectionId) {
                const name = collectionNameFromDir(dir);
                const collection = await createCollection(sdk(), state, name);
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ query, limit }) => {
            const state = loadSyncState(dir);
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filename, refresh }) => {
            const state = loadSyncState(dir);
            const entry = state.files[filename];
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filename, folder }) => {
            const state = loadSyncState(dir);
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filename, newName }) => {
            const state = loadSyncState(dir);
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filename, label, value }) => {
            const state = loadSyncState(dir);
            const mediaId = getMediaId(state, filename);
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filenames, label, value }) => {
            const state = loadSyncState(dir);
            const results = [];
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filename, prompt }) => {
            const state = loadSyncState(dir);
            const mediaId = getMediaId(state, filename);
            if (!mediaId) return notSyncedResponse(filename);
    ...
    server.tool(
        'find_similar',
        'Find images in the folder that are visually similar to a given file. Uses Coreviz embeddings. The reference file must be uploaded to CoreViz first.',
        {
        ...
        safeCloud(ctx, async ({ filename, limit }) => {
            const state = loadSyncState(dir);
            if (!state.collectionId) return noCollectionResponse();
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filename, prompt, aspectRatio, model }) => {
            const state = loadSyncState(dir);
            const mediaId = getMediaId(state, filename);
            if (!mediaId) return notSyncedResponse(filename);
    ...
    server.tool(
        ...
        safeCloud(ctx, async ({ filename, prompt, label, options, multiple }) => {
            const state = loadSyncState(dir);
            const mediaId = getMediaId(state, filename);
            if (!mediaId) return notSyncedResponse(filename);
